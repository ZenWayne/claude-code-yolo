...
# Parallel download using HTTP range requests (curl only).
# Splits the file into N chunks and downloads them concurrently, then
# concatenates the parts. Falls back to single-connection download if
# the server doesn't report a content length, doesn't honor ranges,
# or if curl is not the active downloader.
download_file_parallel() {
    local url="$1"
    local output="$2"
    local connections="${PARALLEL_CONNECTIONS:-8}"

    if [ "$DOWNLOADER" != "curl" ]; then
        # wget has no native multi-connection support; fall back.
        download_file "$url" "$output"
        return $?
    fi

    # Probe content length and range support via HEAD.
    local headers size accept_ranges
    if ! headers=$(curl -fsSLI "$url" 2>/dev/null); then
        download_file "$url" "$output"
        return $?
    fi
    size=$(printf '%s\n' "$headers" | awk 'BEGIN{IGNORECASE=1} /^content-length:/ {gsub(/\r/,""); v=$2} END{print v}')
    accept_ranges=$(printf '%s\n' "$headers" | awk 'BEGIN{IGNORECASE=1} /^accept-ranges:/ {gsub(/\r/,""); v=$2} END{print v}')

    # Need a known size, range support, and a file big enough to bother splitting (>=1 MiB).
    if [ -z "$size" ] || ! [[ "$size" =~ ^[0-9]+$ ]] || [ "$size" -lt 1048576 ] \
       || [ "$accept_ranges" = "none" ] || [ -z "$accept_ranges" ]; then
        download_file "$url" "$output"
        return $?
    fi
    ...
    local chunk_size=$(( (size + connections - 1) / connections ))
    ...
    for ((i = 0; i < connections; i++)); do
        ...
        curl -fsSL -r "${start}-${end}" -o "$tmpdir/part.$(printf '%04d' "$i")" "$url" &
        pids+=("$!")
    done
    ...
    if [ "$fail" -ne 0 ]; then
        rm -rf "$tmpdir"
        # Retry as a single-connection download before giving up.
        download_file "$url" "$output"
        return $?
    fi
...
if ! download_file_parallel "$GCS_BUCKET/$version/$platform/claude" "$binary_path"; then
